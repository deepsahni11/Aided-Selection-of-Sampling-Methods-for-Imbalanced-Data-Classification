...
test_y = np.load("../Code_github/y_test_precision_datasets_bootstrap.npy", allow_pickle = True)


print("Training x" , len(training_x))
rs_test = []
rs_train = []
training_loss = []
test_loss = []
...
for b in range(len(training_x)): # number of bootstrapped samples
    ...
    np.save("../Code_github/rmse_sampling_for_bootstrap_" + str(b) + ".npy" , rmse_sampling)
